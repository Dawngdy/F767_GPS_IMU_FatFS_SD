...
void DynamicModeIdentify(PIMU_DATA_T pImuData)                //gty？ 为什么仅仅依靠IMU数据，而不是依靠GPS速度等数据.....
{
	...
	if (g_GINavInfo.StaticCount > 65535)
		g_GINavInfo.StaticCount = 65535;
}
...
void InitInstallMat(PIMU_DATA_T pImuData)
{
	...
	if (SmoothCount < INSTALL_SMOOTHOR_LEN )
	return;
	...
	{
		if (ABS(gravity*INS_UPDATE_RATE - GRAVITY_CONST )  < 5)                      //gty 这个判断一定需要吗？如果进入不了，系统如何处理？
		{
      
			g_GINavInfo.AccRightFlag=1;
